          ...
          #  print("OK")
            ...
            for plant in plants:
                ...
                for resource in environmental_niche[tuple(plant.pos)]:
                    ...
                    for niche_element in plant.niche:#plant.niche['niche']:
                         
                        if resource == niche_element: 
                        
                            index_resource_plant =plant.niche.index(resource) #plant.niche["niche"].index(resource) #should be niche element for plant but because they are equals no problem
                            index_environmental_resource = environmental_niche[tuple(plant.pos)].index(resource)
                            f_res = np.exp((1/(plant.v[index_resource_plant]))*np.cos(2*np.pi*(ques[tuple(plant.pos)][index_environmental_resource]-plant.q_ast[index_resource_plant])))/i0(1/plant.v[index_resource_plant])
            ...
            elimate_plants.extend(losers)

    return elimate_plants, set(competitive_ab_sps)
